            ...
            if(lopta.pohlad==3){ context.drawImage(pohlad[3] ,lopta.x-15, lopta.y-15, 30, 30) }   
                ...
                context.closePath()
                 ...
                 if(lopta.naraz == false){
                       move(lopta.direction, lopta)
                    }else{
                        check_point("Ouch, Skús to ešte!") 
                    }
    }
...
function priserka_draw(){
    ...
    for (i in priserky) {
            ...
            CiarkyVsPriserky(priserka) //collision, the most hard for count

                move(priserka.direction, priserka)
